...
import pickle
from utils import print_and_log, get_log_files, TestAccuracies, loss, aggregate_accuracy, aggregate_accuracy_sta, send_email
...
from models.base.few_shot_clip_two_trans import MDMF_CLIPFSAR
#from models.base.few_shot_clip_two_trans_vit_plot import CNN_OTAM_CLIPFSAR
...
def parse_command_line():
    ...
    # 经常需要修改
    parser.add_argument("--training_iterations", "-i", type=int, default=45020, help="Number of meta-training iterations.")
    # 经常需要修改
    parser.add_argument("--num_test_tasks", type=int, default=5000, help="number of random tasks to test on.")
    parser.add_argument('--test_iters', nargs='+', type=int, default=[10000, 30000], help='iterations to test at. Default is for ssv2 otam split.')
    ...
    #####################数据集和pt保存路径#############
    parser.add_argument("--dataset", choices=["ssv2", "ssv2_cmn", "kinetics", "hmdb", "ucf"], default="ssv2_cmn", help="Dataset to use.")
    ...
    parser.add_argument("--checkpoint_dir", "-c", default="./checkpoint_ssv2_cmn/diss_ps/", help="Directory to save checkpoint to.")

    parser.add_argument("--test_checkpoint_iter", type=int, default=20000, help="Path to model to load and test.")
    parser.add_argument("--test_model_path", "-m", default="./checkpoint_ssv2_cmn/diss_ps/", help="Path to model to load and test.")
    parser.add_argument("--test_model_only", type=bool, default=True, help="Only testing the model from the given checkpoint")

    #####################下面的是消融参数##############
    parser.add_argument("--lambdas", type=int, default=[0, 1, 1], help="temporal_set")  #1是监督分类loss 2是蒸馏loss
    parser.add_argument('--is_pps', type=bool, default=True)
    parser.add_argument('--dist_direction', default='bid', help='distillation direction')  #bid up2down down2up
    parser.add_argument('--dist_condition', type=int, default=[1, 1],  help='distillation condition') #t-compare, v-compare  #这 3*2*2 一共是12种组合
...
class Learner:
    ...
    def __init__(self):
        ...
        self.accuracy_fn_sta = aggregate_accuracy_sta
        # 根据参数定义优化器
        if self.args.opt == "adam":
            ...
            for p in self.model.backbone.parameters():
                p.requires_grad = True
            # for p in self.model.context_local.parameters():
            #     p.requires_grad = True
    ...
    def init_model(self):
        model = MDMF_CLIPFSAR(self.args, self.cfg)
        model = model.cuda()
    ...
    def train_task(self, task_dict):
        ...
        for k in task_dict.keys():
            if k != "support_frames_info" and k != "query_frames_info":
        ...
        self_logits = model_dict['self_logits']
        ...
        self_logits = self_logits / self.args.tasks_per_batch

        task_loss_total = lambdas[0] * loss0 + lambdas[1] * loss1 + lambdas[2] * self_logits

        task_loss_total.backward(retain_graph=False)
        # target_logits_total.backward(retain_graph=False)

        # return target_logits_total,0.5
    ...
    def test(self, num_episode):
        ...
        with torch.no_grad():
            ...
            for task_dict in self.video_loader:
                ...
                print("--> task_accuracy:{}".format(accuracy))

                self.accuracy_fn_sta(target_logits_total, target_labels, real_target_labels, statics)
        ...
        for key in statics:
            correct_num = 0
            class_sample_num = len(statics[key])
            for sample in statics[key]:
                       if True == sample.item():
                           correct_num = correct_num + 1
            wrong_num = class_sample_num - correct_num
            statics_output[key] = (correct_num/class_sample_num)

        import json
        statics_output = json.dumps(statics_output)
        with open('/home/guofei/Projects/statics_output.json', 'w') as f:
            f.write(statics_output)
        f.close()
